...
AUTOMATED_SCORES_PATH = (
    f"../evaluations/automated/{ASPECT}/sentence_level/scores_based_on_emd"
)
STYLE_DISTRIBUTIONS_PATH = (
    f"../evaluations/automated/{ASPECT}/sentence_level/style_distributions"
)
...
def load_style_distributions(path):
    ...
    return np.array(distributions["fasttext"]).astype("float64"), np.array(
        distributions["textcnn"]
    ).astype("float64")
...
def account_for_direction(
    input_target_style_probability, output_target_style_probability
):
    """
...
def calculate_direction_corrected_emd(
    input_distribution, output_distribution, target_style_class
):
    """
    Calculate Earth Mover's Distance (aka Wasserstein distance) between
    ...
    direction_factor = account_for_direction(
        input_distribution[target_style_class], output_distribution[target_style_class]
    )
...
def display_correlation_tables():
    ...
    input_text_fasttext_distr, input_text_textcnn_distr = load_style_distributions(
        f"{STYLE_DISTRIBUTIONS_PATH}/input_distributions.npz"
    )
    ...
    for model in MODEL_TO_PARAMS:
        ...
        for val in param_values:
            ...
            output_text_fasttext_distr, output_text_textcnn_distr = (
                load_style_distributions(path)
            )
            ...
            fasttext_target_style_scores = np.concatenate(
                [
                    extract_scores_for_style_class(
                        output_text_fasttext_distr, positive_style_class
                    )[:split_point],
                    extract_scores_for_style_class(
                        output_text_fasttext_distr, negative_style_class
                    )[split_point:],
                ]
            )
            ...
            textcnn_target_style_scores = np.concatenate(
                [
                    extract_scores_for_style_class(
                        output_text_textcnn_distr, positive_style_class
                    )[:split_point],
                    extract_scores_for_style_class(
                        output_text_textcnn_distr, negative_style_class
                    )[split_point:],
                ]
            )

            automated_scores_over_model_params.setdefault(
                "fasttext (target style scores)", []
            ).extend(fasttext_target_style_scores)
            automated_scores_over_model_params.setdefault(
                "textcnn  (target style scores)", []
            ).extend(textcnn_target_style_scores)
            ...
            for i in range(number_of_scores):
                ...
                fasttext_intensity = calculate_direction_corrected_emd(
                    input_text_fasttext_distr[i],
                    output_text_fasttext_distr[i],
                    target_style_class,
                )
                textcnn_intensity = calculate_direction_corrected_emd(
                    input_text_textcnn_distr[i],
                    output_text_textcnn_distr[i],
                    target_style_class,
                )
            ...
            automated_scores_over_model_params.setdefault(
                "fasttext (EMD-based)", []
            ).extend(fasttext_intensities)
            automated_scores_over_model_params.setdefault(
                "textcnn  (EMD-based)", []
            ).extend(textcnn_intensities)

        correlation_tables = calculate_correlations(
            automated_scores_over_model_params, turk_scores_over_model_params
        )
