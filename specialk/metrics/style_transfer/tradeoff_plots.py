...
class TradeoffPlotParameters(object):
    ...
    def __init__(self, model_name, color, line_style, marker, marker_size=5):
        ...
        self.content_preservation_scores = load_corpus_level_scores(
            model_name, "content_preservation"
        )
        self.style_transfer_intensities = load_corpus_level_scores(
            model_name, "style_transfer_intensity"
        )
    ...
    def get_aspect_values(self, aspect):
        ...
        values = [
            parameters_to_values[key] for key in sorted(parameters_to_values.keys())
        ]
...
def plot_best_fit_curve(x, y, color, line_style, order):
    ...
    (handler,) = plt.plot(
        *p.linspace(n=100, domain=(0, 1)), color=color, linestyle=line_style
    )
    return handler


def plot_aspect_tradeoffs(
    model_data,
    aspect1,
    aspect2,
    axis_labels,
    ...
    if len(model_data) == 0:
        raise ValueError(
            "Please enter a list of at least one valid TradeoffPlotParameters object."
        )
    ...
    for md in model_data:
        ...
        for x, y in aspect_pair_values:
            plt.plot(
                ...
                y,
                color="black",
                label=md.legend_title,
                marker=md.marker,
                markersize=md.marker_size,
            )

        # plot best fit line
        legend_handler = plot_best_fit_curve(
            aspect1_values,
            aspect2_values,
            color=color,
