...
parser.add_argument("-num_filters", default=100, type=int, help="Number of CNN filters")
parser.add_argument(
...
def eval(model, criterion, data, vocab_size):
    ...
    for i in range(len(data)):
        ...
        inp = (
            src[0] % vocab_size
        )  # Size is seq_len x batch_size, type: torch.cuda.LongTensor, Variable
        ...
        if len(opt.gpus) >= 1:
            one_hot = Variable(
                torch.cuda.FloatTensor(
                    src[0].size(0), src[0].size(1), vocab_size
                ).zero_()
            )
        else:
            one_hot = Variable(
                torch.FloatTensor(src[0].size(0), src[0].size(1), vocab_size).zero_()
            )
...
def main():
    ...
    trainData = onmt.Dataset(
        dataset["train"]["src"], dataset["train"]["tgt"], opt.batch_size, opt.gpus
    )
