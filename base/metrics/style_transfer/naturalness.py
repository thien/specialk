...
class UnigramBasedClassifier(NaturalnessClassifier):
    ...
    def score(self, texts):
        ...
        scores = distribution[
            :, 1
        ]  # column 1 represents probability of being 'natural'
        return scores
...
def display_agreements():
    ...
    human_absolute_judgments_for_input_texts = np.load(
        f"../evaluations/human/{ASPECT}/input_texts.npz"
    )["absolute"]
    ...
    for model in MODEL_TO_PARAMS:
        ...
        automated_scores_for_input_texts = np.load(
            f"{AUTOMATED_EVALUATION_BASE_PATH}/input_texts.npz"
        )
        unigram_scores_for_input_texts = automated_scores_for_input_texts[
            "unigram"
        ].item()[model]
        neural_scores_for_input_texts = automated_scores_for_input_texts[
            "neural"
        ].item()[model]
        ...
        for val in param_values:
            ...
            human_judgments = load_turk_scores(
                ASPECT, model, param_name, str_val, npy_file=False
            )
            human_absolute_judgments_for_output_texts = human_judgments["absolute"]
            human_absolute_judgments = generate_judgments(
                human_absolute_judgments_for_input_texts,
                human_absolute_judgments_for_output_texts,
            )
            human_relative_judgments = format_relative_judgments(
                human_judgments["relative"]
            )

            # load classifier scores of output texts
            automated_scores_for_output_texts_path = (
                f"{AUTOMATED_EVALUATION_BASE_PATH}/{model}_{param_name}_{str_val}.npz"
            )
            automated_scores_for_output_texts = np.load(
                automated_scores_for_output_texts_path
            )

            # count number of agreements between what classifiers mark as more natural and what humans mark as more natural
            unigram_scores_for_output_texts = automated_scores_for_output_texts[
                "unigram"
            ]
            unigram_judgments = generate_judgments(
                unigram_scores_for_input_texts, unigram_scores_for_output_texts
            )
            agreements["unigram_and_absolute"] += np.sum(
                unigram_judgments == human_absolute_judgments
            )
            agreements["unigram_and_relative"] += np.sum(
                unigram_judgments == human_relative_judgments
            )

            neural_scores_for_output_texts = automated_scores_for_output_texts["neural"]
            neural_judgments = generate_judgments(
                neural_scores_for_input_texts, neural_scores_for_output_texts
            )
            agreements["neural_and_absolute"] += np.sum(
                neural_judgments == human_absolute_judgments
            )
            agreements["neural_and_relative"] += np.sum(
                neural_judgments == human_relative_judgments
            )
...
if __name__ == "__main__":
    ...
    # load data
    negative_to_positive_transfers = load_dataset(
        f"../transfer_model_outputs/{model}/{param}_{val}/sentiment.test.0.tsf"
    )
    positive_to_negative_transfers = load_dataset(
        f"../transfer_model_outputs/{model}/{param}_{val}/sentiment.test.1.tsf"
    )
    output_texts = merge_datasets(
        negative_to_positive_transfers, positive_to_negative_transfers
    )
    ...
    scores = neural_classifier.score(output_texts)
    print(len(scores))
