...
do_quality_to_popular = True # if true: do quality to popular, else do popular to quality
...
def printdiff(seqs, srclabel, tgtlabel, block_width=50, spacing_block_len=2):
    ...
    for i in range(len(seqs)):
        ...
        if seqs[i][0:2] == "@@":
            continue
        srcline, tgtline = diff_filter(seqs[i].strip()), diff_filter(seqs[i].strip(), "tgt")
        ...
        for i in range(len(left)):
            ...
            left_line_before = len(left_line.replace("[-","").replace("-]",""))-spacing_block_len

            left_line = left_line.replace("[-", " "+src_before).replace("-]", src_after)+src_after
        ...
        for i in range(len(right)):
            ...
            right_line = right_line.replace("{+", tgt_before).replace("+}", tgt_after)+tgt_after
            right[i] = right_line
...
def do_forward_translation(n):
    ...
    french_sequences = [googt.translate(x,  src='en', dest='fr').text for x in tqdm(n, desc="Forward Translation")]
...
def load_transformer_model(decoder_path):
    args = load_args()
    args.data = os.path.join(basedir,"models/fren_bpe_gm_mk2_articles_mk1/gm_mk2_rebase_quality.pt")
    args.model = "transformer"
    args.epochs = "1"
    opt = args.parse_args(["-data", args.data, "-model", args.model, "-epochs", args.epochs])
    opt.batch_size = 1
    opt.checkpoint_encoder = os.path.join(basedir,"models/fren_bpe_gold_master_mk2/encoder_epoch_3.chkpt")
...
popular_translator, popular_model = load_transformer_model("models/fren_bpe_gm_mk2_articles_mk1/decoder_popular_epoch_3.chkpt")
quality_translator, quality_model = load_transformer_model("models/fren_bpe_gm_mk2_articles_mk1/decoder_quality_epoch_4.chkpt")

print("Loaded Transformer")

dl = Downloader()
sites = ['https://www.thesun.co.uk/news/politics/',
...
testurl="https://www.theguardian.com/world/2019/sep/07/hurricane-dorian-survivors-bahamas"
...
def handle_style_transfer(testurl):
    ...
    sequences = [x.replace('”', '"').replace('“', '"').replace("’", "'").replace("\'", "'").lower() for x in results[0][2]['content'] if x]
    ...
    vocab =  os.path.join(basedir,"models/fren_bpe_gm_mk2_articles_mk1/gm_mk2_rebase_quality.pt")
    ...
    test_loader, max_token_seq_len, is_bpe, decoder = encmodel.load_testdata(test_txt, vocab)
