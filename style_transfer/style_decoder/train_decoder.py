...
def NLLLoss(vocabSize):
    ...
    crit = nn.NLLLoss(weight, reduction='mean')
    if opt.gpus:
        crit.cuda()
    return crit
...
def memoryEfficientLoss(outputs, targets, model, criterion1, criterion2, eval=False):
    ...
    if len(opt.gpus) >= 1:
        ...
        curr_zeros = torch.cuda.FloatTensor(abs(opt.sequence_length - linear.size(0)),
    ...
    loss1 = criterion1(generator_outputs.view(-1, generator_outputs.size(2)), targets.view(-1))
    class_tgt = Variable(class_tgt)
    loss2 = criterion2(class_outputs, class_tgt)
    ...
    return loss1.item(), loss2.item(), grad_output, num_correct


def eval(model, criterion1, criterion2, data):
    total_loss = 0
    total_words = 0
    total_num_correct = 0
    ...
    for i in range(len(data)):
        ...
        loss, _, _, num_correct = memoryEfficientLoss(
                outputs, targets, model, criterion1, criterion2, eval=True)
        total_loss += loss
...
def trainModel(model, trainData, validData, dataset, optim):
    ...
    criterion1 = NLLLoss(dataset['dicts']['tgt'].size())
    criterion2 = BCELoss()
    ...
    def trainEpoch(epoch):
        ...
        for i in range(len(trainData)):
            ...
            loss, closs, gradOutput, num_correct = memoryEfficientLoss(
                     outputs, targets, model, criterion1, criterion2)
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        valid_loss, valid_acc = eval(model, criterion1, criterion2, validData)
