...
parser.add_argument('-seed', default=0,
                    help="Randomiser seed used to shuffle the dataset.")
...
with open(opt.source_a) as a, open(opt.source_b) as b:
    for lang1, lang2 in tqdm(zip(a,b), total=num_ents, desc="read", disable=not opt.verbose):
...
for i in tqdm(range(len(datatypes)), desc="write", disable=not opt.verbose):
    ...
    with open(a_filename, p) as s, open(b_filename, p) as t:
        for s_txt, t_txt in tqdm(group_dataset, desc=group_name, disable=not opt.verbose):
            s.write(s_txt)
            t.write(t_txt)