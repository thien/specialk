...
def load_args():
    ...
    parser.add_argument('-src_vocab',
                        help="Path to an existing source vocabulary pt file.")
...
def main():
    ...
    if opt.src_vocab:
        ...
        src_bpe = bpe_encoder(vocab_size=opt.vocab_size, pct_bpe=opt.pct_bpe, ngram_min=1, UNK=Constants.UNK_WORD, PAD=Constants.PAD_WORD, word_tokenizer=bpe_parse)
    ...
    for g in tqdm(dataset, desc="Trimming Sequences"):
        ...
        for i in range(len(sequences)):
            ref_seq = raw[g]['src'][i]
            bpe_seq = sequences[i]
            dataset[g]['src'][i] = reclip(ref_seq, bpe_seq, src_bpe, opt.max_word_seq_len-2)
