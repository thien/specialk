...
MODEL_TO_PARAM_NAMES = {"ARAE": "lambda", "CAAE": "rho", "DAR": "gamma"}
...
def load_turk_scores(aspect, model, param, param_val, npy_file=True):
    ...
    return np.load(
        f"../evaluations/human/{aspect}/{model}_{param}_{param_val}.{filetype}"
    )
...
def calculate_corpus_level_scores(model, aspect):
    ...
    automated_sentence_level_scores_base_path = (
        f"../evaluations/automated/{aspect}/sentence_level"
    )
