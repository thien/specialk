...
from specialk.models.legacy.recurrent_model import RecurrentModel as recurrent
...
def load_args():
    ...
    parser.add_argument(
        ...
        required=True,
        help="""Either a recurrent (seq2seq model) or a transformer.""",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="""Dimension size of the hidden layers of the transformer.""",
    )
    parser.add_argument(
        "-d_k",
        type=int,
        default=64,
        help="""Key vector dimension size.""",
    )
    parser.add_argument(
        "-d_v",
        type=int,
        default=64,
        help="""Value vector dimension size.""",
    )
    parser.add_argument(
        "-n_head",
        type=int,
    ...
    parser.add_argument(
        "-embs_share_weight",
        action="store_true",
        help="""If enabled, allows the embeddings of the encoder
                        and the decoder to share weights.""",
    )
    parser.add_argument(
        "-proj_share_weight",
        action="store_true",
        help="""If enabled, allows the projection/generator 
                        to share weights.""",
    )
    parser.add_argument(
        "-label_smoothing",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="""Feed the context vector at each time step as additional input (via concatenation with the word embeddings) to the decoder.""",
    )
    parser.add_argument(
        "-max_grad_norm",
        type=float,
    ...
    parser.add_argument(
        "-curriculum",
        action="store_true",
        help="""For this many epochs, order the minibatches based on source sequence length. Sometimes setting this to 1 will increase convergence speed.""",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-freeze_encoder",
        action="store_true",
        help="""If enabled, freezes learning of the encoder.""",
    )
    parser.add_argument(
        "-freeze_decoder",
        action="store_true",
        help="""If enabled, freezes learning of the decoder.""",
    )
